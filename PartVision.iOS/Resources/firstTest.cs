...
namespace PartVision.iOS {
	...
	/// </summary>
	public class firstTestInput : NSObject, IMLFeatureProvider
	{
		...
		public firstTestInput (CVPixelBuffer data)
		{
			if (data == null)
	...
	/// </summary>
	public class firstTestOutput : NSObject, IMLFeatureProvider
	{
		static readonly NSSet<NSString> featureNames = new NSSet<NSString> (
		...
		public firstTestOutput (NSDictionary<NSObject, NSNumber> loss, string classLabel)
		{
			if (loss == null)
	...
	/// </summary>
	public class firstTest : NSObject
	{
		readonly MLModel model;

		public firstTest ()
		{
			var url = NSBundle.MainBundle.GetUrlForResource ("firstTest", "mlmodelc");
			NSError err;

			model = MLModel.Create (url, out err);
		}

		firstTest (MLModel model)
		{
			this.model = model;
		}

		public static firstTest Create (NSUrl url, out NSError error)
		{
			if (url == null)
			...
			return new firstTest (model);
		}

		/// <summary>
		/// Make a prediction using the standard interface
		/// </summary>
		/// <param name="input">an instance of firstTestInput to predict from</param>
		/// <param name="error">If an error occurs, upon return contains an NSError object that describes the problem.</param>
		public firstTestOutput GetPrediction (firstTestInput input, out NSError error)
		{
			...
			return new firstTestOutput (lossValue, classLabelValue);
		}
		...
		/// <param name="error">If an error occurs, upon return contains an NSError object that describes the problem.</param>
		public firstTestOutput GetPrediction (CVPixelBuffer data, out NSError error)
		{
			var input = new firstTestInput (data);
